...
function handle_bank_operation($operation, $url = null) {
	...
	switch ( $operation ) {
		...
		case "receipts":
			...
			$args["actions"] = array(array("Receipt", "/org/business/business-post.php?operation=create_invoice_bank&id=%s"));
			...
			if ($ids) $query .= " and id in (" . comma_implode($ids) . ")";
		...
		case "payments":
			...
			$args["header_fields"] = array("Id", "Date", "Description", "Amount");
			$args["actions"] = array(array("Mark payment", "/org/business/business-post.php?operation=create_invoice_bank&id=%s"));
...
//			       " order by date desc limit $rows_per_page offset $offset";
	...
	//		print gui_hyperlink("Older", add_to_url("page", $page + 1));
		...
		case 'import_from_file':
			...
			$I = new Importer();
			$fields = null;$fields = array(); $fields['account_id'] = get_param('selection');
...
// $args["debug"] = (get_user_id() == 1);
	...
	$fields = GetArg($args, "fields", array("id", "date", "description", "out_amount", "in_amount", "balance"));
