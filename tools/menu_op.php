...
if ( ! $m->isMaster() ) {
//	add_command( $table, $row, $col, "edit_shop_orders", "סנכרן איזורים", "/tools/multi-site/sync-data.php?table=wp_woocommerce_shipping_zone_locations&operation=update&source=4" );
//	add_command( $table, $row, $col, "edit_shop_orders", "סנכרן סוגי משלוח", "/tools/multi-site/sync-data.php?table=wp_woocommerce_shipping_zone_methods&operation=update&source=4" );
	add_command( $table, $row, $col, "edit_shop_orders", "סנכרן מידע", "/tools/multi-site/sync-from-master.php" );
}
while ( $row < $max_row ) {
	$table[ $row ++ ][ $col ] = "";
}
...
add_command( $table, $row, $col, "edit_pricelist", "משימות פעילות", "tasklist/c-get-all-tasklist.php?preset=1", "tasks" );
