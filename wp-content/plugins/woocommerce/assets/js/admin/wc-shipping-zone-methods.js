		...
		var $table          = $( '.wc-shipping-zone-methods' ),
			...
			ShippingMethod       = Backbone.Model.extend({
				...
				logChanges: function( changedRows ) {
					...
					if ( typeof changedRows.min_order !== 'undefined' ) {
						changes.min_order = changedRows.min_order;
					}

					if ( typeof changedRows.default_price !== 'undefined' ) {
						changes.default_price = changedRows.default_price;
					}
			...
			ShippingMethodView = Backbone.View.extend({
				...
				initialize: function() {
					...
					$( document.body ).on( 'input change', '#zone_name, #zone_locations, #zone_postcodes, #min_order, $default_price', { view: this }, this.onUpdateZone );
