...
print Core_Html::gui_header(1, "השלמת תמונות לאתר");

foreach ($categs as $categ){
	$missing_pictures = get_missing_pictures($categ);
	if (count($missing_pictures)){
		print Core_Html::gui_header(2, get_term_name($categ));

		foreach ($missing_pictures as $prod_id){
			// https://fruity.co.il/wp-admin/post.php?post=7015&action=edit
			print Core_Html::GuiHyperlink(get_product_name($prod_id), "/wp-admin/post.php?post=$prod_id&action=edit") . "<br/>";
		}
	}
}
