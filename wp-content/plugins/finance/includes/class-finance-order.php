...
//	}
	...
	function create_delivery() {
		...
		$report   = Core_Html::GuiHeader( 1, "Create delivery note for order $order_id" );
		...
		if ( $id > 0 ) {
			...
			$d = new Finance_Delivery( $id );
			if ( $d->getCustomerType() ) {
				$report .= "<script> let client_type = " . $d->getCustomerType() . ";</script>";
			}
			$d->$report .= Deliveries( Finance_DocumentType::delivery, Finance_DocumentOperation::edit, false );
			...
			if ( 0 and SqlQuerySingleScalar( "select order_is_group(" . $order_id . ")" ) == 1 ) {
				...
				if ( count( $order_ids ) == 0 ) {
					$report .= "אין הזמנות ללקוח הזמנות במצב טיפול<br/>";
					die ( 1 );
				}
				$report .= " הזמנות " . CommaImplode( $order_ids );
				$d      = delivery::CreateFromOrders( $order_ids );
				$report .= $d->OrderInfoBox( $order_ids, false, "יצירת תעודת משלוח ל" );
			} else {
				$report .= $O->infoBox( false, "יצירת תעודת משלוח ל" );

				$d = Finance_Delivery::CreateFromOrder( $order_id );
			}
			$d->$report .= self::Deliveries( Finance_DocumentType::delivery, Finance_DocumentOperation::create, false );
			$report     .= "</form>";

			$report .= '<button id="btn_calc" onclick="calcDelivery()">חשב תעודה</button>';
			...
			$user_id = 1;
			$report  .= '<button id="btn_add" onclick="addDelivery(0)">אשר תעודה</button>
    <button id=\"btn_addline\" onclick=\"addLine(\'' . Fresh::getPost() . ", 0, $user_id)\" הוסף שורה</button>" .
