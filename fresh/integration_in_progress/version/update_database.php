...
function add_version($version, $force = false)
{
	...
	if (! $force){
		$exists = SqlQuerySingleScalar("select count(*) from ${db_prefix}versions where version = '$version'");
		if ($exists >= 1) {
...
//			var_dump($description);
	...
	if (is_string($description))
		SqlQuery("insert into ${db_prefix}versions(version, description, install_date) values ('$version', '$description', now())");
	return true;
}
...
{
$db_prefix = GetTablePrefix();
	SqlQuery("drop table ${db_prefix}cities");
}
...
function version28()
{
	...
	SqlQuery("alter table ${db_prefix}versions add install_date date;");

	print Core_Html::gui_header(1, "zone_times");
	SqlQuery("alter table ${db_prefix}paths change zones zones_times longtext null;");
	SqlQuery("alter table ${db_prefix}missions change zones zones_times longtext null;");

	print Core_Html::gui_header(1, "function template_last_task");
	SqlQuery("CREATE FUNCTION 	template_last_task(_template_id int)
	 RETURNS date
BEGIN
...
END;");
	...
	SqlQuery( "create table ${db_prefix}paths (
...
function version27()
{
	...
	SqlQuery("ALTER TABLE ${db_prefix}projects ADD company int not null default 1;");

	print Core_Html::gui_header(1, "add is_active to project");
	SqlQuery("ALTER TABLE ${db_prefix}projects ADD is_active int(1) not null default 1;");
...
function version26()
{
	...
	SqlQuery("drop function worker_teams");
	SqlQuery("CREATE FUNCTION 	worker_teams(_user_id int)
	 RETURNS TEXT
BEGIN
...
print Core_Html::gui_header(1, "month_with_index");
	SqlQuery("drop function month_with_index;");
	SqlQuery("CREATE FUNCTION month_with_index(_date date) RETURNS VARCHAR(20) 
	BEGIN
	...
	SqlQuery("ALTER TABLE ${db_prefix}task_templates ADD team int not null default 1;");

	print Core_Html::gui_header(1, "template check info");
	SqlQuery("alter table ${db_prefix}task_templates add last_check datetime;");
	print Core_Html::gui_header(1, "template working hours");
	SqlQuery("alter table ${db_prefix}task_templates add working_hours varchar(50);");
	print Core_Html::gui_header(1, "worker projects");
	SqlQuery("drop function worker_projects");
	SqlQuery("CREATE FUNCTION 	worker_projects(_user_id int)
	 RETURNS TEXT
BEGIN
...
END;");
	...
	SqlQuery("ALTER TABLE ${db_prefix}tasklist modify preq varchar(200) null;");

	print Core_Html::gui_header(1, "is_active");
	SqlQuery("ALTER TABLE ${db_prefix}task_templates ADD is_active bit not null default 1;");
	SqlQuery("ALTER TABLE ${db_prefix}tasklist ADD is_active bit not null default 1;");
	SqlQuery("ALTER TABLE ${db_prefix}tasklist ADD team int not null default 1;");
	SqlQuery("ALTER TABLE ${db_prefix}working ADD is_active bit not null default 1;");

	print Core_Html::gui_header(1, "save mission path");
	SqlQuery("ALTER TABLE ${db_prefix}missions ADD path varchar(4000);");

	print Core_Html::gui_header(1, "part of basket");
	SqlQuery("ALTER TABLE ${db_prefix}delivery_lines ADD part_of_basket int;");

	print Core_Html::gui_header(1, "bug management");

	SqlQuery("ALTER TABLE ${db_prefix}tasklist ADD task_type int;");

	SqlQuery( "create table ${db_prefix}task_type (
	id INT NOT NULL AUTO_INCREMENT
		PRIMARY KEY,
    	description longtext CHARACTER SET utf8 NULL,
	    meta_fields varchar(200))" );

	SqlQuery( "create table ${db_prefix}task_meta (
	...
	SqlQuery("ALTER TABLE ${db_prefix}working_hours ADD comment varchar(200);");
...
BEGIN
  ...
  END; ");
	...
	SqlQuery("ALTER TABLE ${db_prefix}multisite ADD user varchar(100);");
	SqlQuery("ALTER TABLE ${db_prefix}multisite ADD password varchar(100);");
...
function version21()
{
	...
	if (! TableExists("ibank_transaction_types")) {
		SqlQuery("ALTER TABLE ${db_prefix}bank ADD transaction_type int;");

		SqlQuery( "create table ${db_prefix}bank_transaction_types (
	id INT NOT NULL AUTO_INCREMENT
		PRIMARY KEY,
    	description VARCHAR(40) CHARACTER SET utf8 NULL,
	    part_id int(11))" );
	...
	SqlQuery("ALTER TABLE ${db_prefix}payments ADD accountants varchar(100);");

	SqlQuery("ALTER TABLE ${db_prefix}delivery_lines ADD a int;");
...
function version20()
{
	...
	SqlQuery("ALTER TABLE ${db_prefix}working ADD company_id int;");

	SqlQuery("ALTER TABLE ${db_prefix}working rename worker_id user_id");

	print Core_Html::gui_header(1, "management");

	if (! TableExists("working_teams")) SqlQuery("create table ${db_prefix}working_teams (
	id INT NOT NULL AUTO_INCREMENT	PRIMARY KEY,
...
function check() {
	...
	SqlQuery( "alter table ${db_prefix}tasklist collate 	utf8_general_ci" );
	$result = SqlQuery( "show create table ${db_prefix}tasklist" );
...
function basic()
{
	...
	{
		SqlQuery("create table ${db_prefix}projects
(
...
$db_prefix = GetTablePrefix();
	if (! TableExists ("working"))
		SqlQuery("create table ${db_prefix}working
(
	id int auto_increment,
...
");
	if (! TableExists("company")) SqlQuery("create table ${db_prefix}company
(
	id int not null AUTO_INCREMENT PRIMARY KEY,
...
");
	if (! TableExists("tasklist"))  SqlQuery( "CREATE TABLE `${db_prefix}tasklist` (
...
`creator` INT(11), 
 ...
 `owner` INT(11),
  PRIMARY KEY (`id`)
	...
	SqlQuery( "ALTER TABLE ${db_prefix}tasklist
...
function version18()
{
	...
	SqlQuery( "drop function task_active_time;" );
	SqlQuery( "
...
" );
	...
	SqlQuery( "drop function task_template_time;" );
	SqlQuery( "
...
");
	...
	SqlQuery("drop function get_product_name");
	SqlQuery("create
...
");
	...
	SqlQuery("create function delivery_receipt( _del_id int) returns integer
	BEGIN
...
");
	...
	SqlQuery("drop function project_name");
	SqlQuery("create function project_name( _project_id int) returns varchar(100) charset utf8
	BEGIN
		declare _name varchar(100) charset utf8;
...
");
	print "pay_date<br/>";
	SqlQuery("ALTER TABLE ${db_prefix}business_info ADD pay_date date;");
	print "month name<br/>";
	SqlQuery("drop function month_with_index;");
	SqlQuery("CREATE FUNCTION month_with_index(_date date) RETURNS VARCHAR(20) 
	BEGIN
	...
	SqlQuery( "alter table ${db_prefix}task_templates " .
	          " add owner int(11), " .
	          " add creator int(11); ");

	print "project_count";
	SqlQuery("drop function project_count");
	SqlQuery( "create function project_count (_project_id int, _owner_id int) returns int   
BEGIN
declare _count int;
...
function version17() {
	...
	SqlQuery( "ALTER TABLE ${db_prefix}projects ADD project_priority INT(11);" );
	print "auto_order_day<br/>";
	SqlQuery( "ALTER TABLE ${db_prefix}suppliers ADD auto_order_day INT(11);" );
	SqlQuery( "drop function bank_amount_to_link" );
	SqlQuery( "create function bank_amount_to_link (_line_id int) returns float   
BEGIN
declare _sum int;
declare _amount float;
...
END;" );
	...
	SqlQuery( "alter table ${db_prefix}suppliers
	...
	print "bank_account";
	SqlQuery( "create table ${db_prefix}bank_account
(
	id int not null auto_increment
		primary key,
...
" );
	print "bank_lines<br/>";
	SqlQuery( "create table ${db_prefix}bank_lines
(
	id int not null auto_increment
		primary key,
	line_id int null,
...
" );
	...
	SqlQuery( "drop view ${db_prefix}products_draft" );

	SqlQuery( "create view ${db_prefix}products_draft as 
	SELECT
    `wp_posts`.`ID`                    AS `ID`,
...
function version16() {
	...
	SqlQuery( "ALTER TABLE ${db_prefix}business_info
ADD net_amount DOUBLE;
" );

	SqlQuery( "ALTER TABLE ${db_prefix}supplies
ADD picked BIT  
" );
...
function drop_projects()
{
	...
	SqlQuery("drop table ${db_prefix}projects");
}
...
function get_versions()
{
	...
	$versions = SqlQueryArray("select version, description, install_date from ${db_prefix}versions");
	if (is_string($versions) and substr($versions, 0, 5) === "Error"){
		SqlQuery( "create table ${db_prefix}versions (
	id INT NOT NULL AUTO_INCREMENT
		PRIMARY KEY,
		version varchar(20),
    	description VARCHAR(40) CHARACTER SET utf8 NULL,
...
function version29()
{
	...
	SqlQuery("ALTER TABLE ${db_prefix}projects ADD manager int not null default 1;");
	...
	SqlQuery("drop function bank_balance");
	SqlQuery("create function bank_balance(_account_id int) returns float
BEGIN
...
");
	...
	SqlQuery("create function client_id_from_delivery(del_id int) returns integer
BEGIN
  declare _order_id int;
  declare _user_id int;
  declare _display varchar(50) CHARSET utf8;
...
BEGIN
  ...
  END; ");
	...
	SqlQuery("create table ${db_prefix}bank
(
	id int auto_increment
		primary key,
...
");
	...
	SqlQuery("create function bank_last_transaction(_account_id int)
	returns date
BEGIN
		declare _result date;
	...
	return _result;	   
END	");
	SqlQuery("drop function account_name");
	SqlQuery("CREATE FUNCTION 	account_name(_account_id int)
...
");
	...
	SqlQuery("alter table ${db_prefix}bank_account
    add owner int(11)");
	...
	SqlQuery("alter table ${db_prefix}missions add accepting bit default 1;");


	print Core_Html::gui_header(1, "Herbal");

	SqlQuery( "create table me_clients ( " . // Client record
	          "id INT NOT NULL AUTO_INCREMENT	PRIMARY KEY, " .
	          "user_id int(11), " .
	          "symptoms longtext); " );
...
function category_views()
{
	...
	SqlQuery("create view ${db_prefix}categories as select `wp_terms`.`term_id`    AS `term_id`,
