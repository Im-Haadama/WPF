...
while ( $row = mysqli_fetch_row( $result ) ) {
	$order_id  = $row[0];
	$post_date = $row[1];
	$row_text  = "<td><a href=\"http://store.im-haadama.co.il/fresh/orders/get-order.php?order_id=" . $order_id . "\">" . $order_id . "</a></td><td>" . $post_date . "</td>";
...
while ( $row = mysqli_fetch_row( $result ) ) {
	...
	$line = "<td> " . $prod_name .
	        "</td><td><a href = \"http://store.im-haadama.co.il/fresh/get-orders-per-item.php?prod_id=" . $prod_id . "\">" . $prod_quantity . "</a></td>";
